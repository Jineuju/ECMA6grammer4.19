...
이 결과값의 객체를 "iterable object" 기존의 객체와 아주 살짝 다릅니다

next()는 제너레이터의 주요 메소드입니다
next()로 제너레이터를 호출하면 가장 가까이에 있는 yield를 만날때까지
실행하다가 yield를 만나면 실행을 멈추고 산출하고자하는 값을 반환합니다
{value: 'test1', done: false} <-- 이런 식으로 yield로 불러온 함수의 Value와
함수의 실행 여부를 도출합니다
value 산출값, 해당함수의 결과값
done 함수의 코드가 실행되었는지에 대한 여부
